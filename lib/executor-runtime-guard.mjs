...
import { EXECUTORS_DIR, SAFETY_STATE_FILE, RUNTIME_EVENTS_LOG } from './config.mjs';

const ROOT_DIR = join(dirname(fileURLToPath(import.meta.url)), '..');
const CONFIG_PROFILES_FILE = join(ROOT_DIR, 'config', 'executor-safety-profiles.json');
// Single source. The runtime/ duplicate was removed (editing it silently had no
// effect because this file always won); a missing file falls back to the
// DEFAULT_POLICIES above rather than to a path that no longer exists.
const DEFAULT_POLICY_FILE = CONFIG_PROFILES_FILE;
...
const DEFAULT_STATE_FILE = SAFETY_STATE_FILE;
const DEFAULT_EVENTS_LOG_FILE = RUNTIME_EVENTS_LOG;
...
export function sanitizeAuditEvent(value) {
  if (Array.isArray(value)) return value.map((v) => sanitizeAuditEvent(v));
  if (!value || typeof value !== 'object') return value;
  const out = {};
  for (const [k, v] of Object.entries(value)) {
    if (FORBIDDEN_AUDIT_KEYS.has(k.toLowerCase())) continue;
    out[k] = sanitizeAuditEvent(v);
  }
  return out;
}

...
const DEFAULT_STATE_FILE = join(ROOT_DIR, 'runtime', 'executor-safety-state.json');
...
export class ExecutorRuntimeGuard {
  ...
  #saveState() {
    try {
      ...
      writeFileSync(this.stateFile, JSON.stringify(obj, null, 2));
    } catch { /* best effort */ }
  }
  ...
  getCircuitState(executorType) {
    ...
    if (c.state === 'OPEN_COOLDOWN' && c.cooldown_until && this.now() >= c.cooldown_until) {
      c.state = 'HALF_OPEN';
      this.#saveState();
    }
    return { ...c };

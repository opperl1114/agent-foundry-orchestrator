...
import {RUNTIME_DIR,TASKS_DIR_DEFAULT} from './config.mjs';
...
export function readWorkbench(id,root=ROOT){
 try{return JSON.parse(readFileSync(join(RUNTIME_DIR,'workbench',valid(id)+'.json'),'utf8'));}
...
export function changeWorkbench(id,fn,root=ROOT){
 const dir=join(RUNTIME_DIR,'workbench');mkdirSync(dir,{recursive:true});const path=join(dir,valid(id)+'.json');let lock;
...
export async function awaitExperience(id,run,root=ROOT,{sleep=ms=>new Promise(r=>setTimeout(r,ms))}={}){
 ...
 for(;;){
  ...
  waiting=pending.id;
  try{if(JSON.parse(readFileSync(join(TASKS_DIR_DEFAULT,id+'.json'),'utf8')).state==='CANCELLED')return 'cancelled';}catch(e){if(e.code!=='ENOENT')throw e;}
  await sleep(1000);
 }
}

...
import { acceptanceBinding, verifyAcceptanceBinding, normalizeAcceptanceCmd } from './acceptance.mjs';
import { LOCKS_DIR, RUNTIME_DIR, TASKS_DIR_DEFAULT } from './config.mjs';
...
export class Scheduler {
  constructor({
    ...
    tasksDir = null,            // override for isolated tests (defaults to AF_TASKS_DIR / ROOT/tasks)
    locksDir = null,
    runtimeDir = null,
    ...
    this.tasksDir = tasksDir ?? TASKS_DIR_DEFAULT;
  ...
  enqueue(taskOrFile) {
    ...
    } else if (!verifyAcceptanceBinding(task).bound) {
      // The file already exists (a pre-placed definition, or a task being
      // re-queued). Enqueue is where the Control Plane takes ownership of the
      // definition, so adopt it by writing the anchor once. Everything after
      // this point verifies it, so deleting the field later is still caught.
      // Deliberately a metadata write: it must not advance state_version, which
      // a recovery plan in flight is compared against.
      const onDisk = readTaskFile(this.#taskFile(id));
      onDisk.acceptance_binding = acceptanceBinding(onDisk);
      saveTaskAtomic(this.#taskFile(id), onDisk);
    }
    this.queue.push(id);
  ...
  async shutdown({ timeoutMs = 5000, mode = 'cancel', signal = 'SIGTERM' } = {}) {
    ...
    // 2. Reclaim acceptance children too. They are not executor runs, so the
    //    cancellation path below never saw them; a hung acceptance command would
    //    otherwise outlive the shutdown.
    try {
      const { terminateActiveAcceptances } = await import('./acceptance.mjs');
      const acc = terminateActiveAcceptances({ signal });
      if (acc.requested > 0) this.#event('acceptance_children_terminated', { count: acc.requested, signal });
    } catch { /* acceptance module unavailable: nothing to reclaim */ }

    // 3. Terminate active executor child processes via existing cancellation mechanism

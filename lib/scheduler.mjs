...
export class Scheduler {
  ...
  cancelTask(taskId) {
    ...
    if (wasActive && entry?.adapterType && entry.activeRunId) {
    ...
    if (!TERMINAL_STATES.has(task.state)) {
      ...
      saveTaskAtomic(this.#taskFile(taskId), task);
    }
  ...
  #start(taskId, { resume }) {
    ...
    const entry = {
      ...
      adapterType: null, activeRunId: null,
      fallbacks,
      // populated by the run chain on every executor start so cancelTask can
      // target the precise active run identity (never "latest process")
      onRunStart: (runId, adapterType) => { entry.activeRunId = runId; entry.adapterType = adapterType; },
    ...
    for (;;) {
      ...
      try {
        ...
        if (!bindingCheck.ok) {
          ...
          saveTaskAtomic(this.#taskFile(taskId), fresh);
        ...
        try {
          ...
          if (!TERMINAL_STATES.has(t.state)) {
            if (this.shutdownRequested) {
              ...
              saveTaskAtomic(this.#taskFile(taskId), t);
            }
          }

...
export function instrumentAdapter(id,adapter,base=root){
 const wrap=method=>async(...args)=>{
  ...
  if(!/^[A-Za-z0-9_-]+$/.test(taskId))return adapter[method](...args);
  ...
  try{const result=await adapter[method](...args);if(result.status==='completed'){const stopped=await awaitExperience(taskId,runId,base);if(stopped==='cancelled')result.status='cancelled';if(stopped==='changes_requested'){atomic(path,{...activity,status:'changes_requested',finished_at:new Date().toISOString()});return wrap(method)(...args);}}atomic(path,{...activity,status:result.status||'finished',finished_at:new Date().toISOString()});return result;}
  catch(e){atomic(path,{...activity,status:'failed',finished_at:new Date().toISOString()});throw e;}
 };
 return {...adapter,...(adapter.run?{run:wrap('run')}:{}),...(adapter.resume?{resume:wrap('resume')}: {})};
}

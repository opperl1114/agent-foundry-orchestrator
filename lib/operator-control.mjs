...
export function instrumentAdapter(id,adapter,base=root){
 ...
 const wrap=(method, orig)=>async(...args)=>{
  ...
  const inputDir=join(RUNTIME_DIR,'operator-input',taskId);
  ...
  const path=join(RUNTIME_DIR,'operator-activity',taskId,`${runId}.json`);
  atomic(path,activity);
  for(const input of inputs)atomic(join(RUNTIME_DIR,'operator-received',taskId,`${input.value.id}-${runId}.json`),{input_id:input.value.id,run_id:runId,received_at:new Date().toISOString()});

...
// ---------------------------------------------------------------- zcode
// zcode-af -> zcode.cjs / zcode CLI (Z.ai ADE Node bundle)
// Non-interactive mode: --mode yolo --prompt "<prompt>" --json
...
export const ZcodeAdapter = {
  ...
  exact_resume: true,
  health() {
    const launcher = process.env.ZCODE_LAUNCHER || ZCODE_LAUNCHER;
    return {
      executor_type: 'zcode',
      launcher,
      governance: 'canonical via zcode-af launcher',
      ok: exists(launcher) || (ZCODE_CJS && existsSync(ZCODE_CJS)) || !!onPath('zcode'),
    };
  },
  async run(capsule) {
    return zcodeExec([], capsule);
  },
  async resume(sessionRef, capsule) {
    if (!sessionRef) throw new Error('zcode resume requires an explicit session_id');
    return zcodeExec(['--resume', sessionRef], capsule, sessionRef);
  },
  cancel(runId) { return cancelRun(runId, { graceMs: 4000 }); },
};

async function zcodeExec(preArgs, capsule, expectSession = null) {
  const launcher = process.env.ZCODE_LAUNCHER || ZCODE_LAUNCHER;
  const timeoutMs = capsule.timeout_ms ?? 600000;
  const cwd = capsule.workspace_dir || capsule.fixture_dir || process.cwd();
  const runId = capsule.runId ?? null;
  const taskId = capsule.task_id ?? null;
  const protectActiveProcess = capsule.protect_active_process ?? true;
  const idleTimeoutMs = capsule.idle_timeout_ms ?? 120000;

  const promptText = buildTaskPrompt(capsule);
  const args = [
    ...preArgs,
    '--mode', 'yolo',
    '--cwd', cwd,
  ...
  return finalizeZcode(r, capsule, expectSession);
}

function finalizeZcode(r, capsule, expectSession = null) {
  let sessionRef = expectSession;
  let resultText = '';
  let structured = null;
  ...
  if (r.stdout) {
    try {
      ...
      if (parsed.sessionId || parsed.session_id) sessionRef = parsed.sessionId || parsed.session_id;
      if (parsed.result || parsed.text || parsed.message) resultText = parsed.result || parsed.text || parsed.message;
      structured = { parsed, raw: r.stdout };
    } catch {
      const lines = r.stdout.split('\n').filter(Boolean);
      for (const line of lines) {
        try {
          const obj = JSON.parse(line);
          if (obj.sessionId || obj.session_id) sessionRef = obj.sessionId || obj.session_id;
          if (obj.text || obj.result) resultText = obj.text || obj.result;
        } catch { /* skip non-JSON line */ }
      }
    }
  }

  if (!resultText) resultText = r.stdout || '';
  if (!structured) structured = { result: resultText, raw: r.stdout || '' };

  const ok = r.exit_code === 0 && !r.cancelled;
  const failMsg = r.spawn_error
    ? `spawn error: ${r.spawn_error}`
    : (ok ? null : (r.stderr || `exit ${r.exit_code}`));
  ...
  return baseResult({
    executor_type: 'zcode',
    assigned_role: capsule.assigned_role,
    status: ok ? 'completed' : (r.cancelled ? 'cancelled' : 'failed'),
    session_ref: sessionRef,
    structured_result: structured,
    exit_code: r.exit_code,
    started_at: r.started,
...
export function selectExecutor(preference, { requiresMcp, adapters = ADAPTERS } = {}) {
  ...
  for (const id of ['antigravity', 'claude', 'codex', 'cline', 'zcode']) {
    const a = adapters[id];

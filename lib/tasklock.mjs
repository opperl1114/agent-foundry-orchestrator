...
export function isLockStale(lock, now = Date.now()) {
  ...
  if (lock.lease_expires_at && Date.parse(lock.lease_expires_at) < now) return true;
...
export function renewTaskLock(locksDir, taskId, lock, leaseMs = 15 * 60_000) {
  // heartbeat: extend our own lease in place (only the owner may renew)
  ...
  current.lease_expires_at = new Date(Date.now() + leaseMs).toISOString();
  writeFileSync(lockPath(locksDir, taskId), JSON.stringify(current, null, 2));
  return current;
}

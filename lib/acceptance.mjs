...
export function runAcceptance(task) {
  ...
  return new Promise((resolve) => {
    ...
    if (child.pid !== undefined) {
      activeAcceptances.set(child.pid, child);
      // Durable handle: the in-process reclamation above only covers a graceful
      // shutdown, and the timeout lives in this process. If this process is
      // SIGKILLed the child is reparented to init with nothing bounding it, so
      // leave a discoverable record (see reapOrphanedAcceptances).
      handleRunId = writeAcceptanceHandle({
        taskId: task.task_id,
        pid: child.pid,
        spec,
        cwd,
...
import { CURRENT_NODE_BIN_DIR, RUNS_DIR } from './config.mjs';
...
// ------------------------------------------------------- acceptance handle
// The timeout above lives in THIS process, so a SIGKILLed orchestrator leaves
// its acceptance child reparented to init with nothing to bound it and no way
// to find it. A durable handle makes the child discoverable; the reaper below
// then requires an ORPHANED pid that still matches the recorded command - so it
// can never take a process that a live orchestrator is still managing (that one
// is already bounded by its own timeout).

const ACCEPTANCE_HANDLE_PREFIX = 'ACCEPT-';

function acceptanceHandleFile(runId, runsDir = RUNS_DIR) {
  return join(runsDir, `${runId}.json`);
}

function writeAcceptanceHandle({ taskId, pid, spec, cwd, startedAt, runsDir = RUNS_DIR }) {
  const runId = `${ACCEPTANCE_HANDLE_PREFIX}${taskId ?? 'unknown'}-${Date.now()}`;
  try {
    mkdirSync(runsDir, { recursive: true });
    writeFileSync(acceptanceHandleFile(runId, runsDir), JSON.stringify({
      run_id: runId,
      kind: 'acceptance',
      task_id: taskId ?? null,
    ...
    return null; // discovery is best effort; the timeout still applies
  }
}

function removeAcceptanceHandle(runId, runsDir = RUNS_DIR) {
  if (!runId) return;
  try { unlinkSync(acceptanceHandleFile(runId, runsDir)); } catch { /* already gone */ }
}
...
// /proc/<pid>/stat field 4 is the parent pid. The comm field is parenthesised
// and may itself contain spaces, so parse after the LAST ')'.
...
function cmdlineMatchesHandle(handle) {
  const cmdline = cmdlineOf(handle.pid);
  if (!cmdline) return false;
  const program = String(handle.command ?? '').split('/').pop();
  if (program && !cmdline.includes(program)) return false;
  const firstArg = Array.isArray(handle.args) ? handle.args[0] : undefined;
  if (firstArg && !cmdline.includes(String(firstArg))) return false;
  return true;
}

export function listAcceptanceHandles({ runsDir = RUNS_DIR } = {}) {
  let files = [];
  try {
    files = readdirSync(runsDir).filter((f) => f.startsWith(ACCEPTANCE_HANDLE_PREFIX) && f.endsWith('.json'));
  } catch {
    return [];
  }
  const out = [];
  for (const file of files) {
    let handle;
    try { handle = JSON.parse(readFileSync(join(runsDir, file), 'utf8')); } catch { continue; }
    const alive = pidAlive(handle.pid);
    const parent = alive ? parentPidOf(handle.pid) : null;
    // "Orphaned" means the OWNER is gone - not "reparented to pid 1". Where a
    // subreaper is present (containers, supervisors, some test runners) orphans
    // are adopted by it and never reach pid 1, so the ppid test would simply
    // never fire. Handles without an owner_pid fall back to the ppid heuristic.
    const ownerAlive = handle.owner_pid ? pidAlive(handle.owner_pid) : null;
    out.push({
      ...handle,
      pid_alive: alive,
      parent_pid: parent,
      owner_alive: ownerAlive,
      orphaned: ownerAlive === null ? parent === 1 : !ownerAlive,
      cmdline_matches: alive ? cmdlineMatchesHandle(handle) : false,
    });
  }
  return out;
}

// Reaps only acceptance children that lost their owner (ppid 1) AND whose
// cmdline still proves the pid is the process the handle recorded. Dry run
// unless confirm is set, like pruneTasks.
export async function reapOrphanedAcceptances({ runsDir = RUNS_DIR, confirm = false, graceMs = 2000 } = {}) {
  const reaped = [];
  const skipped = [];
  for (const handle of listAcceptanceHandles({ runsDir })) {
    if (!handle.pid_alive) {
      removeAcceptanceHandle(handle.run_id, runsDir);
      skipped.push({ run_id: handle.run_id, reason: 'pid no longer exists (handle cleaned up)' });
      continue;
    }
    if (!handle.orphaned) {
      const who = handle.owner_pid ? `owner pid ${handle.owner_pid}` : `parent pid ${handle.parent_pid}`;
      skipped.push({ run_id: handle.run_id, reason: `${who} is alive - it still bounds this child via acceptance_timeout_ms` });
      continue;
    }
    if (!handle.cmdline_matches) {
      skipped.push({ run_id: handle.run_id, reason: 'cmdline no longer matches the recorded command (pid reuse) - refusing to signal' });
      continue;
    }
    if (!confirm) {
      skipped.push({ run_id: handle.run_id, reason: `dry run - orphaned pid ${handle.pid} would be terminated (pass --confirm)` });
      continue;
    }
    ...
    removeAcceptanceHandle(handle.run_id, runsDir);
    reaped.push({ run_id: handle.run_id, pid: handle.pid, task_id: handle.task_id, command: `${handle.command} ${(handle.args ?? []).join(' ')}`.trim() });
  }
  return { reaped, skipped, confirm };
}

export function formatAcceptanceReapResult(res) {
  const lines = [res.confirm ? 'acceptance reap (confirmed)' : 'acceptance reap (dry run - nothing was signalled)'];
  if (res.reaped.length === 0) lines.push('  reaped: none');
  for (const r of res.reaped) lines.push(`  reaped pid ${r.pid} (${r.command}) task=${r.task_id ?? '-'}`);
  for (const s of res.skipped) lines.push(`  skipped ${s.run_id}: ${s.reason}`);
  return lines.join('\n');
}

...
export const ZCODE_LAUNCHER = process.env.ZCODE_LAUNCHER ||
  resolveDir('ZCODE_LAUNCHER_DIR', [
    resolve(ROOT_DIR, 'bin/zcode-af'),
    join(HOME, 'bin/zcode-af'),
  ]);

export const ZCODE_CJS = process.env.ZCODE_CJS ||
  (existsSync('/mnt/e/Zcode/resources/glm/zcode.cjs') ? '/mnt/e/Zcode/resources/glm/zcode.cjs'
    : existsSync('E:/Zcode/resources/glm/zcode.cjs') ? 'E:/Zcode/resources/glm/zcode.cjs'
    : '');
...
// 5. Runtime state, locks and task storage
//
// These are PER-INSTANCE paths: every run rewrites them. They are resolved once,
// here, and are env-overridable so a test (or a second operator instance) can
// point all of them at one sandbox instead of writing into the checkout.
// AF_RUNTIME_DIR moves the whole group; the finer-grained variables win over it.
export const RUNTIME_DIR = process.env.AF_RUNTIME_DIR || join(ROOT_DIR, 'runtime');
export const LOCKS_DIR = process.env.AF_LOCKS_DIR || join(ROOT_DIR, 'locks');
export const TASKS_DIR_DEFAULT = process.env.AF_TASKS_DIR || join(ROOT_DIR, 'tasks');
export const RUNS_DIR = process.env.AF_RUNS_DIR || join(RUNTIME_DIR, 'runs');
export const SAFETY_STATE_FILE = process.env.AF_SAFETY_STATE_FILE ||
  join(RUNTIME_DIR, 'executor-safety-state.json');
export const RUNTIME_EVENTS_LOG = process.env.AF_RUNTIME_EVENTS_LOG ||
  join(RUNTIME_DIR, 'executor-runtime-events.jsonl');
